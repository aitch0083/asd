	...
	if(signed_user){
		...
		form.on('end', function(fields, files) {
			...
			_.each(this.openedFiles, function(file, idx){
				...
				var img_name     = 'IMG' + now.format('YYYYMMDDHHmmss') + 'I' + _.padStart(_.random(10000, 99999), 5, '0') + 'U' + signed_user.id + ext;
				var public_path  = config.image_prefix + 'images/uploaded/' + img_name;
				...
				fs.rename(file.path, af_real_path, function(error){
					...
					if(watermark){
						gm(af_real_path).draw(['gravity SouthEast image Over 20,20 125,44 "' + wm_path + '"']).write(af_real_path, function(e){
							if(e){
